            ...
            let displayData = payload.map((object) => {
                ...
                return `
                    <tr>
                        ...
                        <td>
                            <button type="submit" onclick="rolPremium('${String(_id)}')">Premium<button>
                            <button onclick="deleteUser('${String(_id)}')">Eliminar<button>
                        </td>
                    </tr>
                `});
...
function rolPremium(userId) {
    fetch(`http://localhost:8080/api/users/premium/${userId}`, { method: 'PUT' }).then(response => {
        if (!response.ok) {
